...
import domain.{APIAccess, UserInfo}
...
trait DocumentationController extends uk.gov.hmrc.api.controllers.DocumentationController {
  ...
  override def definition(): Action[AnyContent] = Action {
    Ok(views.txt.definition(buildAccess())).withHeaders(("Content-Type" -> "application/json"))
  }
...
object Documentation {
  ...
  def applyTemplate(apiName: String, version: String)(info: UserInfo): Option[Xml] = {
    (apiName, version) match {
      case (`getUserInfo`, `version1_0`) => Some(views.xml.getUserInfo(info))
      case (`getUserInfoPost`, `version1_0`) => Some(views.xml.getUserInfoPost(info))
      case _ => None
    }
  }
