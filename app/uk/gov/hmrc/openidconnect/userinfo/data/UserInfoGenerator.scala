...
trait UserInfoGenerator {
  ...
  val lastNames = List("Deschain", "Dean", "Dean", "Chambers", "Bumbler", "Allgood", "Johns", "Curry", "Whitman", "Delgado", "Flagg", "Bowen")
  ...
  val userInfo = for {
    ...
    dob <- dateOfBirth
    nino <- formattedNino
  } yield UserInfo(name, Some(lastName), middleName, address, Some(dob), Some(nino))
}
